...
class TestServices(unittest.TestCase):
    ...
    def test_inserting_couriers(self):
        c1 = Courier(1, CourierType.foot, [1, 2], [
                     TimeInterval(9, 0, 12, 0),
                     TimeInterval(14, 0, 17, 0)])
    ...
    def test_inserting_orders(self):
        o1 = Order(1, 4.67, 2, [
                TimeInterval(10, 0, 15, 0),
                TimeInterval(20, 10, 21, 40)])
        o2 = Order(2, 11.5, 3, [
            TimeInterval(3, 00, 6, 00),
            TimeInterval(14, 0, 17, 00)])
    ...
    def test_assigning_orders(self):
        c1 = Courier(1, CourierType.foot, [1, 2], [
                     TimeInterval(9, 0, 12, 0), TimeInterval(14, 0, 17, 0)])
        c2 = Courier(2, CourierType.bike, [2, 3], [TimeInterval(10, 0, 16, 0)])
        c3 = Courier(3, CourierType.foot, [2], [TimeInterval(
            1, 30, 2, 10), TimeInterval(7, 0, 13, 0)])
        c4 = Courier(4, CourierType.foot, [1, 2, 3], [
                     TimeInterval(5, 0, 10, 0)])
        c5 = Courier(5, CourierType.car, [1, 3], [
                     TimeInterval(15, 0, 19, 0), TimeInterval(10, 0, 11, 0)])
        ...
        o1 = Order(1, 4.67, 2, [
                TimeInterval(10, 0, 15, 0),
                TimeInterval(22, 10, 22, 40)])
        o2 = Order(2, 11.5, 3, [
                TimeInterval(3, 0, 6, 0),
                TimeInterval(14, 0, 17, 0)])
        o3 = Order(3, 5.44, 2, [
                TimeInterval(10, 0, 12, 0),
                TimeInterval(21, 0, 3, 40),
                TimeInterval(6, 0, 8, 0)])
        o4 = Order(4, 3.33, 3, [
                TimeInterval(6, 0, 7, 0),
                TimeInterval(10, 0, 11, 0),
                TimeInterval(12, 0, 13, 0)])
        o5 = Order(5, 1.5, 1, [
            TimeInterval(6, 10, 7, 40),
            TimeInterval(23, 0, 2, 0)])
    ...
    def test_updating_courier(self):
        ...
        c1 = Courier(1, CourierType.bike, [1, 2], [
                     TimeInterval(9, 0, 12, 0),
                     TimeInterval(14, 0, 17, 0)])
        ...
        o1 = Order(1, 4.5, 1, [
                TimeInterval(6, 0, 7, 0),
                TimeInterval(10, 0, 13, 0)])
        o2 = Order(2, 3.5, 2, [
                TimeInterval(0, 0, 3, 0),
                TimeInterval(15, 0, 15, 30)])
        o3 = Order(3, 3.7, 1, [
        ...
        c1 = services.get_courier_by_id(c1.courier_id)
        c1.working_hours = [TimeInterval(
            4, 0, 6, 0), TimeInterval(12, 30, 13, 30)]
        ...
        c2 = Courier(2, CourierType.bike, [1, 2], [
                     TimeInterval(9, 0, 12, 0),
                     TimeInterval(14, 0, 17, 0)])
    ...
    def test_calculating_sallary_and_rating(self):
        ...
        o3 = Order(3, 5.44, 2, [
            TimeInterval(10, 0, 12, 0),
            TimeInterval(21, 0, 3, 40), TimeInterval(6, 0, 8, 0)])
        o4 = Order(4, 3.33, 3, [
            TimeInterval(6, 0, 7, 0),
            TimeInterval(10, 0, 11, 0),
            TimeInterval(12, 0, 13, 0)])
        ...
        # 15 минут
        time = datetime.now().replace(microsecond=0) + timedelta(0, 60 * 15)
        services.complete_order(c1.courier_id, 6, time)
        # 35 минут
        time = datetime.now().replace(microsecond=0) + timedelta(0, 60 * 35)
        services.complete_order(c1.courier_id, 5, time)

        # 3 развоз на машине + пеший развоз = 9 * 500 + 2 * 500
        self.assertEqual(services.calcuate_courier_sallary(
            c1.courier_id), 11 * 500)

        # 50 / 2 = 25 минут
        self.assertEqual(
            services.calculate_courier_rating(c1.courier_id), 3.54)
...
class TestDAL(unittest.TestCase):
    ...
    def test_intersection_of_time(self):
        ...
        b1 = dal.try_assign_order(dal.prepare_list_of_intervals(
            t1), dal.prepare_list_of_intervals(t2))
        self.assertFalse(b1)
        ...
        b2 = dal.try_assign_order(dal.prepare_list_of_intervals(
            t3), dal.prepare_list_of_intervals(t4))
        self.assertTrue(b2)

        t5 = [
            TimeInterval(3, 30, 4, 40),
            TimeInterval(6, 0, 9, 0),
            TimeInterval(11, 0, 12, 50)]
        t6 = [
            TimeInterval(1, 20, 2, 59),
            TimeInterval(7, 20, 8, 33),
            TimeInterval(17, 30, 19, 20)]
        t5 = [(e.start_time, e.end_time) for e in t5]
        t6 = [(e.start_time, e.end_time) for e in t6]

        b3 = dal.try_assign_order(dal.prepare_list_of_intervals(
            t5), dal.prepare_list_of_intervals(t6))
        self.assertTrue(b3)
        ...
        b4 = dal.try_assign_order(dal.prepare_list_of_intervals(
            t7), dal.prepare_list_of_intervals(t8))
        self.assertFalse(b4)
        ...
        b5 = dal.try_assign_order(dal.prepare_list_of_intervals(
            t9), dal.prepare_list_of_intervals(t10))
        self.assertTrue(b5)
